...
router.post('/', noRunningTests, (req, res, next) => {
  ...
  Q.addNewAssessment(assessment)
   ...
   .then(() => {
     salesforce.login()
     .then(() => {
       return salesforce.submitCodingChallenge(req.user.salesforce_id, req.body.oppId, req.body.code, req.body.moveForward, req.body.stage, 'Python_Challenge_Passed__c');
     })
   })
   .catch((err) => {
     honeybadger.notify(err);
